...
CREATE OR REPLACE VIEW ACCOUNT_VIEW AS
  SELECT ACT.ID, USERNAME, BIRTHDAY, GENDER, UNIT, HEIGHT, M.NAME MULTIPLIER, M.AMOUNT MULTIPLIER_AMOUNT, WEEKDAY, ACCOUNT_PKG.GETAGE(BIRTHDAY) AGE, ACTIVE_SINCE
  FROM ACCOUNT ACT
...
CREATE OR REPLACE VIEW ACCOUNT_WEIGHT_VIEW AS
  SELECT ID, USERNAME, WEIGHT, WEIGHT_DATE, MULTIPLIER_AMOUNT AMOUNT, WEEKDAY, AGE, HEIGHT, UNIT, GENDER, ACTIVE_SINCE
...
CREATE OR REPLACE TRIGGER TR_DELETE_ACCOUNT_ALL
BEFORE DELETE
  ON ACCOUNT
  FOR EACH ROW
DECLARE
BEGIN
  DELETE FROM ACCOUNT_SETTING WHERE ACCOUNTID=:OLD.ID;
  DELETE FROM WEIGHT WHERE ACCOUNT_ID=:OLD.ID;
EXCEPTION
  WHEN OTHERS THEN
    RAISE;
    ROLLBACK;
