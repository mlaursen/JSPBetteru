...
CREATE OR REPLACE PACKAGE BODY DAILY_INTAKE_PKG AS
  ...
  PROCEDURE FILTERFROM(PID IN INTEGER, PSTART IN DATE, PCURSOR OUT SYS_REFCURSOR)
  IS
  BEGIN
    OPEN PCURSOR FOR
      SELECT *
      FROM (SELECT *
            FROM DAILY_INTAKE
            WHERE ACCOUNT_ID=0 AND INTAKE_DATE >= TO_DATE('10-FEB-14')
            ORDER BY INTAKE_DATE)
      WHERE ROWNUM <= 7;
  END;
  ...
  BEGIN
    ISQL := 'SELECT V.ID, V.ACCOUNT_ID, TDEE,CALORIE_CHANGE, FAT_MULTIPLIER, 
                    CARB_MULTIPLIER, PROTEIN_MULTIPLIER, MEAL_IDS, V.INTAKE_DATE, WEIGHT
