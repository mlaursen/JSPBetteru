...
CREATE OR REPLACE PACKAGE BODY DAILY_INTAKE_PKG AS
  ...
  BEGIN
    IF PDATE IS NOT NULL THEN
      OPEN PCURSOR FOR
        SELECT DI.ACCOUNT_ID,TDEE,INTAKE_DATE,CALORIE_CHANGE,FAT_MULTIPLIER,CARB_MULTIPLIER,PROTEIN_MULTIPLIER
        FROM DAILY_INTAKE DI
        INNER JOIN MIFFLIN_ST_JOER MSJ
        ON DI.ACCOUNT_ID=MSJ.ACCOUNT_ID AND INTAKE_DATE=WEIGHT_DATE
        WHERE DI.ACCOUNT_ID=PID AND INTAKE_DATE=PDATE
        ORDER BY INTAKE_DATE;
    ELSE
      OPEN PCURSOR FOR
        SELECT DI.ACCOUNT_ID,TDEE,INTAKE_DATE,CALORIE_CHANGE,FAT_MULTIPLIER,CARB_MULTIPLIER,PROTEIN_MULTIPLIER
