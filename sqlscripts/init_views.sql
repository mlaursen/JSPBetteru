...
CREATE OR REPLACE VIEW MIFFLIN_ST_JOER AS
  SELECT ID ACCOUNT_ID, 
         ROUND(AMOUNT*(NEW_WEIGHT+HEIGHT-AGE+EXTRA),2) TDEE,
         WEIGHT,
         WEIGHT_DATE
  FROM (SELECT ID,
               10*WEIGHT/DECODE(UNIT,'IMPERIAL',2.2,1) NEW_WEIGHT,
               6.25*HEIGHT*DECODE(UNIT,'IMPERIAL',2.54,1) HEIGHT,
               AGE*5 AGE,
               DECODE(GENDER,'MALE',5,-161) EXTRA,
               AMOUNT,
               WEIGHT,
               WEIGHT_DATE
...
CREATE OR REPLACE VIEW HARRIS_BENEDICT AS
  SELECT ID ACCOUNT_ID,
         ROUND(AMOUNT*(EXTRA+NEW_WEIGHT+HEIGHT-AGE),2) TDEE,
         WEIGHT,
         WEIGHT_DATE
  FROM (SELECT ID,
               WEIGHT*DECODE(GENDER,'MALE',6.23,4.35) NEW_WEIGHT,
...
CREATE OR REPLACE VIEW DAILY_INTAKE_VIEW AS
  ...
  LEFT JOIN DAILY_MEAL_INTAKE DMI
...
CREATE OR REPLACE VIEW MIFFLIN_ST_JOER_ACCOUNT_VIEW AS
  SELECT ID,
         ...
         PROTEIN_MULTIPLIER
  FROM DAILY_INTAKE DI
  LEFT JOIN MIFFLIN_ST_JOER MSJ
  ON DI.ACCOUNT_ID=MSJ.ACCOUNT_ID AND TRUNC(INTAKE_DATE)=TRUNC(WEIGHT_DATE)
...
CREATE OR REPLACE VIEW HARRIS_BENEDICT_ACCOUNT_VIEW AS
  SELECT ID,
         ...
         PROTEIN_MULTIPLIER
  FROM DAILY_INTAKE DI
  LEFT JOIN HARRIS_BENEDICT HB
  ON DI.ACCOUNT_ID=HB.ACCOUNT_ID AND TRUNC(INTAKE_DATE)=TRUNC(WEIGHT_DATE)
  ORDER BY DI.ACCOUNT_ID, INTAKE_DATE
WITH READ ONLY;